...
""" DATA SAVE PATHS """
SEQ_FOLDER = "output/sequences/{:02}"
...
def create_save_dirs(seq_id):
    seq_root = SEQ_FOLDER.format(seq_id)
    os.makedirs(POSE_FOLDER, exist_ok=True)
    for folder in FOLDERS:
        os.makedirs(os.path.join(seq_root, folder), exist_ok=True)

    lidar_path = osp.join(seq_root, LIDAR_PATH)
    label_path = osp.join(seq_root, LABEL_PATH)
    image_path = osp.join(seq_root, IMAGE_PATH)
    calib_path = osp.join(seq_root, CALIBRATION_PATH)
    time_path = osp.join(seq_root, TIME_PATH)
    pose_path = osp.join(POSE_FOLDER, POSE_PATH)
    paths = {'lidar_path': lidar_path,
...
class World(object):
    ...
    def __init__(self, carla_world, hud, args, save_paths):
        """Constructor method"""
        self._args = args
        ...
        self._weather_index = random.choice(range(len(self._weather_presets)))
        self._actor_filter = args.filter
        self.restart(args, save_paths)
    ...
    def restart(self, args, save_paths):
        """Restart the world"""
        # setup weather
        # self._set_weather(self._weather_index)
    ...
    def _setup_traffic(self, args):
        ...
        vehicle_bps = random.choice(list(bplib.filter('vehicle')), size=NUM_VEHICLES)
        ...
        # # set walkers
        # walker_bps = random.choice(list(bplib.filter('walker')), size=NUM_PEDESTRIANS)
        ...
        # logging.info(f'{len(self.walkers)} pedestrians spawned')

        # # set walker controller
        # controller_bp = bplib.find('controller.ai.walker')
        # for walker in self.walkers:
        #     controller = self.world.spawn_actor(controller_bp, walker.get_transform(), walker)
        #     controller.start()
    ...
    def _set_weather(self, weather_index):
        weather_index %= len(self._weather_presets)
    ...
    def destroy_sensors(self):
        ...
        for sensor in self.camera_manager.record_sensors:
            sensor.destroy()
        self.camera_manager.record_sensors = []
...
class CameraManager(object):
    ...
    def __init__(self, parent_actor, hud):
        ...
        self._record_sensor_transforms = {
            'rgb': carla.Transform(carla.Location(z=CAMERA_HEIGHT_POS)),
            'lidar': carla.Transform(carla.Location(z=LIDAR_HEIGHT_POS))}
        ...
        for item in self.sensor_metas:
            blp = bp_library.find(item[0])
            if item[0].startswith('sensor.camera'):
                blp.set_attribute('image_size_x', str(hud.dim[0]))
                blp.set_attribute('image_size_y', str(hud.dim[1]))
            elif item[0].startswith('sensor.lidar'):
                blp.set_attribute('range', str(MAX_RENDER_DEPTH_IN_METERS))
                blp.set_attribute('rotation_frequency', str(10))
                blp.set_attribute('channels', str(40))
                blp.set_attribute('points_per_second', str(72000))
                blp.set_attribute('upper_fov', str(7))
    ...
    def set_sensor(self, index, notify=True, force_respawn=False):
        """Set a sensor"""
        index = index % len(self.sensor_metas)
        needs_respawn = True if self.index is None else (
            force_respawn or (self.sensor_metas[index][0] != self.sensor_metas[self.index][0]))
        if needs_respawn:
            if self.sensor is not None:
                self.sensor.destroy()
                self.surface = None
            self.sensor = self._parent.get_world().spawn_actor(
                self.sensor_metas[index][-1],
    ...
    def add_record_sensor(self, index, path_pattern):
        ...
        if sensor_type == 'sensor.camera.rgb':
            transform = self._record_sensor_transforms['rgb']
        elif sensor_type == 'sensor.lidar.ray_cast':
            transform = self._record_sensor_transforms['lidar']
    ...
    def _parse_image(weak_self, image):
        ...
        if not self:
            return
        if self.sensor_metas[self.index][0].startswith('sensor.lidar'):
            ...
            image.convert(self.sensor_metas[self.index][1])
...
def game_loop(args):
    ...
    try:
        ...
        world = World(client.get_world(), hud, args, save_paths)
